...
class InstructionMapper(nn.Module):
    ...
    """
    def __init__(self, input_dim=1024, internal_dim=512, spk_emb_dim=256, xvec_dim=192, depth=6):
        super().__init__()
        ...
        # 4. Projection Heads (Chuyển từ Latent chung 512 -> Target Spaces)
        # Chỉ dùng sau khi đã giải ODE xong (Lúc Inference)
        self.head_spk = nn.Linear(internal_dim, spk_emb_dim)   # 512 -> 256
        self.head_xvec = nn.Linear(internal_dim, xvec_dim)     # 512 -> 192
    ...
    def project_to_targets(self, latent_z):
        ...
        latent_z: [Batch, 512] (Kết quả của Flow Matching)
        """
        spk_emb = self.head_spk(latent_z)
        x_vector = self.head_xvec(latent_z)
