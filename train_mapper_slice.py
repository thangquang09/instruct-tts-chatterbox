...
def train_epoch(
    ...
    for batch in pbar:
        ...
        # --- NORMALIZATION ---
        # Update running stats and normalize batch
        norm_spk = mapper_module.spk_normalizer(gt_spk_emb)
        norm_xvec = mapper_module.xvec_normalizer(gt_x_vector)
        
        # Create target latent: [SpkEmb | XVec]
        x_1 = create_fixed_target(norm_spk, norm_xvec)
        ...
        loss = lambda_spk * loss_spk + lambda_xvec * loss_xvec
...
def main():
    parser = argparse.ArgumentParser(description="Train Instruction Mapper with Pure Flow Matching")
    ...
    mapper = InstructionMapper(input_dim=1024, internal_dim=448, spk_emb_dim=256, xvec_dim=192, depth=6).to(device)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if is_main_process():
            logger.info(f"Epoch {epoch}: Val SpkCos = {spk_cos:.4f}, XVecCos = {xvec_cos:.4f}, AvgCos = {avg_cos:.4f}")
